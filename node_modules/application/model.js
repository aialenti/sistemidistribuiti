...
var executeQuery = function(query,callback){
  ...
  connection.query(query, function(err, rows, fields) {
    var error = false;
    if(query.match(/UPDATE/) || query.match(/DELETE/)){
      if(rows.affectedRows == 0){
        var e = new Object;
        e.code = "no_affected_rows";
        errorHandling.handleQueryError(e);
      }
    }
    else{
      if (err) 
...
var searchQuery =  function(data){
  ...
  if(length != 0)
    query += " WHERE ";
  for(i=0;i<length;i++){
    query += data.where[i][0]+"='"+data.where[i][1]+"'";
    if(i<length-1)
      query +=" AND ";
