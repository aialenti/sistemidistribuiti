...
var createNewUser = function(req, res) {
	...
	data.values = [['username', username], ['email', email], ['password', hashedpassword]];
...
var doLogin = function(data, socket) {
	 ...
	 * (funzione di callback): se non si ottiene niente (data.length === 0) vuol
	 * dire che non c'è un user con quell'username nel DB; altrimenti, si
	 * controlla la correttezza della password con bcrypt e si conferma o meno
	 * la validità del login (il messaggio passa alla pagina per il rendering)
	 */
}
...
var manageMatch = function(req, res) {
	model.req = req; // in questo modo non devo inserire req in tutte le
	// chiamate a getParam;
	var query;
	...
	switch (action) {
		case "create" :
			data.values = [["home_team", home_team], ["away_team", away_team], ["matchdays_flag", matchdays_flag], ["matchdays_number", matchdays_number], ["matchdays_season", matchdays_season]]
			query = model.insertQuery(data);
		break;
		case "delete" :
			data.where = [["id", "=", id]];
...
var manageScore = function(req, socket) {
	...
	switch (action) {
		...
		case "update" :
			...
			if (player == false)
				player = ""
			data.values = [["time", time], ["player", player]]
			data.where = [["id", "=", scoreid]];
			query = model.updateQuery(data);
		break;
		default :
...
var manageMatchDay = function(req, res) {
	...
	switch (action) {
		case "create" :
			data.values = [["flag", flag], ["number", number], ["season", season]]
			query = model.insertQuery(data);
		break;
		case "delete" :
			data.where = [["number", "=", number], ["season", "=", season], ["flag", "=", flag]];
			query = model.deleteQuery(data);
		break;
		case "edit" :
			data.values = [["number", "=", new_number], ["season", "=", new_season], ["flag", "=", new_flag]];
			data.where = [["number", "=", number], ["season", "=", season], ["flag", "=", flag]];
			query = model.updateQuery(data);
		break;
		case "activate" :
			data.values = [["past", "1"]];
			data.where = [["number", "=", number], ["season", "=", season]];
			query = model.updateQuery(data);
		break;
		case "deactivate" :
			data.values = [["past", "0"]];
...
var manageTeam = function(req, socket) {
	var session = req.sessionid.express_sid;
	console.log(session);
...
var createSeason = function(req, socket) {
	...
	for ( var i = 0; i < (arrSquadre.length - 1) * 2; i++) {
		if (i < arrSquadre.length - 1)
			...
			flag = "1";
		data.table = "matchdays"
		data.values[i] = [["season", new_season + ""], ["number", "" + i], ["flag", flag]]
	}
	query = model.multiInsertQuery(data);
	model
			.executeQuery(query, function(rows, error, cols) {
				query = "";
				data.table = "matches";
				data.values = new Array();
				var p = 0;
				...
				for ( var i = 0; i < arrSquadre.length - 1; i++) {
					for ( var j = 0; j < arrSquadre.length / 2; j++) {
						...
						data.values[p] = [
						                  ["home_team", "" + ht],
						                  ["away_team", "" + at], 
						                  ["matchdays_season", "" + new_season], ["matchdays_number", matchdays_number + ""], ["matchdays_flag", "0"]];
						p++;
						data.values[p] = [["home_team", "" + at], ["away_team", "" + ht], ["matchdays_season", "" + new_season], ["matchdays_number", "" + (matchdays_number + (arrSquadre.length - 1))], ["matchdays_flag", "1"]];
						p++;
					}
...
var getTeamName = function(req, socket) {
	data = new Object;
	var id = req.id;
	data.where = [["id", "=", id]];
	data.select = ['name'];
...
var getAllTheSeason = function(req, socket) {
	...
	var ret = new Array();
	var season = model.getParam("season");
	var query = "SELECT m.id,m.home_team, m.away_team,m.matchdays_flag,m.matchdays_number,m.matchdays_season,d.past,t.name as home_team_name,t2.name as away_team_name \n\
               FROM matches m,teams t,teams t2,matchdays d \n\
               WHERE m.home_team = t.id AND m.away_team = t2.id AND m.matchdays_season='" + season + "' AND (m.matchdays_number,m.matchdays_season)=(d.number,d.season) ORDER BY m.matchdays_number DESC"; // HERE
	model.executeQuery(query, function(rows, error, cols) {
...
var getMatchday = function(data, socket) {
	// oltre a tornare le scores deve ritornare anche le giornate che non hanno
	// punteggi.
	// Bisogna vedere come utilizzare i risultati. Ci vorrebbe una funzione che
	// becca solo le score
	var matchday = data.matchday_number;
	var season = data.season;
	var query = "SELECT m.id,m.home_team, m.away_team,d.past,m.matchdays_flag,m.matchdays_number,m.matchdays_season,t.name as home_team_name,t2.name as away_team_name \n\
               FROM matches m,teams t,teams t2,matchdays d \n\
               WHERE d.number=m.matchdays_number AND d.season=m.matchdays_season AND d.flag=m.matchdays_flag AND m.home_team = t.id AND m.away_team = t2.id AND m.matchdays_season='" + season + "' AND m.matchdays_number = '" + matchday + "' ORDER BY m.home_team ASC";
	model.executeQuery(query, function(rows2, error, cols) {
				...
				model.executeQuery(query, function(rows, error, cols) {
					...
					returns[0] = rows2;
					returns[1] = rows;
					returns[2] = data.matchid;
					returns[3] = matchday;
					if (data.onlyupdate != true)
						socket.emit("hereSelectMatchdayChange", returns);
					else {
						socket.broadcast.emit("updateScoresView", returns);
