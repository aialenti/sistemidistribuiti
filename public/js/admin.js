...
$(document).ready(function() {
	...
	$("#selectSeason").change(function() {
		// quì va l'emit dell'evento necessario a reperire le info e creare
		// l'interfaccia
	...
	socket.on("errorOnSentData", function(data) {
		...
		switch (data.caller) {
			case "createSeason" :
				...
				var error = "";
				if (data.year.message == "no_four_digits") { // anno stagione
																// errato
					$(group).addClass("error");
					error += "The year must be a four digits number<br/>";
				}
				if (data.year.message.match(/needed/)) { // anno stagione
															// errato
					$(group).addClass("error");
